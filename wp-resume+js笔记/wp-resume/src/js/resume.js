...
require('../../src/css/personcommon.css');
require('../../src/css/resume.css');
...
$(document).ready(function(){
	...
	if(pathName.indexOf("template") > 0 || pathName.indexOf("ppt") > 0) {
		...
		currentSelect(4);
	}else if(pathName.indexOf("print") > 0) {
		currentSelect(5);
	}else {
		currentSelect(0);
	}
...
function check_vip_rank_tips(member_vip,resume_type,webresumeSize){
	$("#sjvipModal").find(".modal-dialog").find("p").hide();
	//判断会员权限
	if(member_vip=="nologin"){
		show_login_modal();
		return 0;
	}
	if(member_vip=="vipexpire"){//VIP过期
		if(resume_type=="wap"){
			$("#sjvipModal").modal("show");
			$("#sjvipModal").find("p.waptips").show();
			return 0;
		}else if(resume_type=="web"){
			$("#sjvipModal").modal("show");
			$("#sjvipModal").find("p.webtips").show();
			return 0;
		}else{
			if(webresumeSize>=3){
				$("#sjvipModal").modal("show");
				$("#sjvipModal").find("p.vipexpire").show();
				return 0;
			}
		}
	}
	if(member_vip=="novip"){
		if(resume_type=="wap"){
			...
			if(webresumeSize>=3){
				$("#sjvipModal").modal("show");
				$("#sjvipModal").find("p.over_3").show();
				return 0;
			}
		}
	}else if(member_vip=="initialvip"){//初级VIP
		if(resume_type=="wap"){
			$("#sjvipModal").modal("show");
			$("#sjvipModal").find("p.waptips").show();
			return 0;
			...
			if(webresumeSize>=10){
				$("#sjvipModal").modal("show");
				$("#sjvipModal").find("p.over_10").show();
				return 0;
			}
		}
	}else if(member_vip=="middlevip"){//进阶VIP
		if(webresumeSize>=50){
			$("#sjvipModal").modal("show");
			$("#sjvipModal").find("p.over_50").show();
			return 0;
		}
	}else{
		if(webresumeSize>=100){
			$("#sjvipModal").modal("show");
			$("#sjvipModal").find("p.over_100").show();
			return 0;
		}
	}
	return 1;
